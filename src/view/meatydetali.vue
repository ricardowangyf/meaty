  ...
  <div>
    ...
    <div class="meatybody">
      ...
      <div class="detali-paragraph">
        <p class="paragraph">
          {{
            showAll ? detali.paragraph : detali.paragraph.slice(0, 20) + "..."
          }}
        </p>
      </div>
      ...
      <div class="footer">
        <div class="button-add">
          <button type="button" @click="sub" class="less">-</button>
          <!--可以用@ 代替v-on-->
  ...
  methods: {
    ...
    shoemore() {
      ...
      if (this.showAll) {
        ...
        let lineHeight =
          this.lineHeight *
          parseInt(getComputedStyle(this.$refs.detali.paragraph).fontSize);
        let maxLines = this.lineClamp;
        let maxHeight = lineHeight * maxLines;
        if (height > maxHeight) {
          this.$refs.detali.paragraph.style.setProperty(
            "max-height",
            maxHeight + "px"
          );
        }
...
.textinput {
  text-align: center;
  background: #ffffff;
  box-shadow: 0 -3px 6px 0 rgb(173 174 179 / 17%),
    0 3px 6px 0 rgb(173 174 179 / 17%);
